...
def build_attribute_info(df_full_cat, category, df_attr):
    ...
    for i, attr in enumerate(attribute_list):
        ...
        attribute_classes[attr] = vals
        attribute_encoders[attr] = {v: idx for idx, v in enumerate(vals)}
        attribute_dims[safe_key(category, attr)] = len(vals)

    return attribute_list, attribute_mapping, attribute_classes, attribute_encoders, attribute_dims


def train_one_epoch(clip_model, model_head, loader, optimizer, criterion):
    model_head.train()
    total_loss = 0
    steps = 0
...
def main():
    ...
    with torch.no_grad():
        dummy = preprocess_train(Image.new("RGB", (224, 224))).unsqueeze(0).to(DEVICE)
    ...
    for category in categories:
        ...
        attr_list, attr_map, attr_classes, attr_encoders, attr_dims = build_attribute_info(
        ...
        for epoch in range(epochs_local):
            loss = train_one_epoch(clip_model, model_head, train_loader, optimizer, criterion)
